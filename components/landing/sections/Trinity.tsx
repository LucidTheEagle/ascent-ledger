...
const LibraryCareerQuote = () => {
  const [libraryColor, setLibraryColor] = useState("#FFFFFF");
  const [careerRevealed, setCareerRevealed] = useState(false);

  useEffect(() => {
    // Sequence: Flicker Red -> Pause -> Reveal Blue
    ...
    return () => {
      clearTimeout(flicker1); clearTimeout(flicker2);
      clearTimeout(flicker3); clearTimeout(flicker4);
...
export function Trinity() {
  const [h2Phase, setH2Phase] = useState<"gray" | "white" | "gradient">("gray");
          ...
          >
            <CardSpotlight className="h-full min-h-[500px] border border-white/10 bg-ascent-obsidian/60">
              <div className="relative z-20 flex flex-col h-full p-6 md:p-8">
                ...
                {/* Chat Area - Structured as Conversation */}
                <div className="flex-1 flex flex-col justify-center w-full">
                   <ChatBubble 
                      role="system" 
                      delay={0.2}
                   ...
                   <ChatBubble 
                      role="user" 
                      delay={0.4}
                      text="I'm 3 years into account management. Hitting quota, but I feel like a cog in the machine."
                   />
                   <ChatBubble 
                      role="system" 
                      delay={0.6}
                      text="Acknowledged. What is the target coordinate?"
                   />
                   <ChatBubble 
                      role="user" 
                      delay={0.8}
                      text="Leading a sales team at a Series B startup."
                   />
                </div>
                ...
                <div className="mt-8 pt-6 border-t border-white/5 flex justify-between items-center">
                  ...
                  <Link
                    href="/login"
                    className="group flex items-center gap-2 text-ascent-blue text-sm font-semibold hover:text-white transition-colors"
                  >
          ...
          >
             <div className="relative h-full w-full rounded-2xl overflow-hidden border border-white/10 bg-ascent-obsidian/80 backdrop-blur-md">
                ...
                <div className="relative z-10 p-6 flex flex-col h-full">
                   ...
                   {/* Replaced Button with simple Tech Link */}
